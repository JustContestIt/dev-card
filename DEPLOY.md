...
1. **База.** [cockroachlabs.cloud](https://cockroachlabs.cloud) → Create Cluster →
   **Basic (Serverless), Free**. В разделе Connect выбери «Connection string»,
   ...
   Строку можно оставить как есть: приложение не завязано на имя базы,
   таблицы создаются в той, что указана в `DATABASE_URL`. Хочешь имя
   поаккуратнее — впиши `dev_card` вместо `defaultdb`, создавать её руками
   не нужно: `prisma migrate deploy` сделает это при первом старте контейнера.
...
## Если деплой упал с `Error: P3009`

```
migrate found failed migrations in the target database
The `2026..._init` migration started at ... failed
```

Значит предыдущая попытка оборвалась посреди миграции (чаще всего — холодный
старт serverless-кластера), и Prisma записала её как проваленную. Сама она из
этого состояния не выйдет: пока запись висит, каждый следующий деплой падает
с P3009, даже если с базой уже всё хорошо.

Самый быстрый выход для свежего деплоя, где данных ещё нет — **направить
приложение на чистую базу**: в `DATABASE_URL` поменяй имя базы (например
`defaultdb` → `dev_card`) и запусти деплой заново. Создавать её руками не надо,
`migrate deploy` сделает это сам.

Если база уже с данными — в SQL-консоли кластера убери запись о провале:

```sql
DELETE FROM "_prisma_migrations" WHERE finished_at IS NULL;
```

Учти: это снимает только блокировку. Если оборвавшаяся миграция успела создать
часть таблиц, повтор упадёт уже на `relation ... already exists` — тогда придётся
удалить и созданные ею объекты (`DROP TABLE ...`), то есть по сути вернуться к
чистой базе. Поэтому для свежего деплоя вариант с новым именем базы проще.

> Начиная с версии с retry в `docker-entrypoint.sh` контейнер сам делает до
> 5 попыток миграции с паузами, так что «холодная» база больше не роняет деплой.
